...
const Button: React.FC<ButtonInterface> = ({ label, action, icon: Icon, leftIcon: LeftIcon, rightIcon: RightIcon, ...props }) => {
  ...
  return (
    <button className='w-auto h-[64px] flex items-center gap-1 px-[35px] border border-[#9e9e9e] hover:border-[#68e0cf] transition-all duration-300 rounded-[64px]' {...props} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
